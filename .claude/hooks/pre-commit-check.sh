#!/usr/bin/env bash
# PreToolUse hook — runs typecheck + tests before the agent is allowed to
# execute `git commit`. Receives the tool-call JSON on stdin; matches on
# the literal substring so we don't depend on jq.
#
