...
export function buildWorkspaceMediator(
  ...
  const datasets = new Map([[DATASET_PATH, dataset]]);
  const workspace = new InMemoryAgentRunWorkspace(outcomes);
  const skillRunner = new InMemorySkillRunner();
  const datasetLoader = new InMemoryDatasetLoader(datasets);

  const defaultResponse: SkillJudgeResponse = { score: 70, observations: ['Stub observation'] };
  const stubJudge = new BehaviorRoutingJudge(dataset, judgeResponses, defaultResponse);

  const handler = new EvaluateSkillHandler(skillRunner, datasetLoader, stubJudge, workspace);
  return new Mediator([handler]);
}
